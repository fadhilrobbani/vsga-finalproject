...
include 'koneksi.php';
$id = $_GET['id'];
$pemesanan = mysqli_query($koneksi, "SELECT * FROM pemesanan WHERE id = '$id'");
$row = mysqli_fetch_array($pemesanan);
?>
<!DOCTYPE html>
<html lang="id">
...
<body>
   ...
   <main>
      ...
      <h3 class="text-center mt-4">Form Edit Pemesanan Paket Wisata</h3>
      <form action="edit.php" method="post">
         <div class="container mt-2">
            ...
            <div class="mb-3">
               ...
               <input type="text" value="<?php echo $row['telepon'] ?>" class="form-control" id="telepon" name="telepon" required>
            </div>
            <div class="mb-3">
               <label for="tanggal" class="form-label fw-bold">Tanggal Pesan:</label>
               <input type="date" value="<?php echo $row['tanggal'] ?>" class="form-control" id="tanggal" name="tanggal" required>
            </div>
            <div class="mb-3">
               <label for="jumlah_hari" class="form-label fw-bold">Waktu Pelaksanaan Perjalanan :</label>
               <input type="number" value="<?php echo $row['jumlah_hari'] ?>" class="form-control" id="jumlah_hari" name="jumlah_hari" required>
            </div>
            <div class="mb-3 d-flex gap-2 flex-column ">
               <p class="text-auto fw-bold mb-0 me-3">Pelayanan Paket Perjalanan:</p>
               <div>
                  <input type="checkbox" <?php echo $row['akomodasi'] ? 'checked' : '' ?> value="1000000" class="form-check-input" id="akomodasi" name="akomodasi">
                  <label for="akomodasi" class="form-check-label me-2">Penginapan (Rp1.000.000)</label>

               </div>
               <div>

                  <input type="checkbox" <?php echo $row['transportasi'] ? 'checked' : '' ?> value="1200000" class="form-check-input" id="transportasi" name="transportasi">
                  <label for="transportasi" class="form-check-label me-2">Transportasi (Rp1.200.000)</label>
               </div>
               <div>

                  <input type="checkbox" <?php echo $row['makan'] ? 'checked' : '' ?> value="500000" class="form-check-input" id="makan" name="makan">
            ...
            <div class="mb-3">
               ...
               <input type="number" value="<?php echo $row['jumlah_peserta'] ?>" class="form-control" id="jumlah_peserta" name="jumlah_peserta" required>
            </div>
            <div class="mb-3">
               <label for="harga_paket" class="form-label fw-bold">Harga Paket Perjalanan:</label>
               <input type="number" value="<?php echo $row['harga_paket'] ?>" class="form-control" id="harga_paket" name="harga_paket" required readonly>
            </div>
            <div class="mb-3">
               <label for="total_tagihan" class="form-label fw-bold">Total Tagihan:</label>
               <input type="number" value="<?php echo $row['total_tagihan'] ?>" class="form-control" id="total_tagihan" name="total_tagihan" required readonly>
            </div>
